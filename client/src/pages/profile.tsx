  ...
  const profileSections = [
    {
      title: "Account",
      items: [
        { id: "personal", icon: User, label: "Personal Information", description: "Update your name, email, and phone" },
        { id: "addresses", icon: MapPin, label: "Addresses", description: "Manage your delivery addresses" },
        { id: "payments", icon: CreditCard, label: "Payment Methods", description: "Add or update payment options" },
      ]
    },
    {
      title: "Preferences",
      items: [
        { id: "settings", icon: Settings, label: "App Settings", description: "Notifications, language, and more" },
        { id: "support", icon: HelpCircle, label: "Help & Support", description: "FAQ, contact us, and feedback" },
      ]
    }
  ];
    ...
    <div className="pb-20 min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm sticky top-0 z-10">
        <div className="px-6 py-4 flex items-center">
          {activeTab ? (
            <Button size="sm" variant="ghost" className="mr-3" onClick={() => setActiveTab(null)}>
          ...
          <h1 className="text-xl font-bold text-gray-800">
            {activeTab === 'payments' ? 'Payment Methods' : 'Profile'}
          </h1>
        </div>
      </div>
                  ...
                  <Card>
                    <CardContent className="p-0">
                      {section.items.map((item, index) => (
                        <button
                          key={item.label}
                          onClick={() => item.id === 'payments' && setActiveTab('payments')}
